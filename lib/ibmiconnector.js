...
class IBMiConnector {
  constructor(settings) {
    ...
    this.pool = new odbc.Pool(settings.dsn); // TODO: this isnt right
  ...
  executeSQL(sql, params, options, callback) {
    console.log(`executeSQL sql is ${sql}`);
 ...
 */
  ...
  autoupdate(mods, cb) {
    debug('IBMiConnector.autoupdate %j', mods);
  ...
  getTableStatus(model, cb) {
    ...
    this.query(columnSQL, (error1, tableInfo) => {
      ...
      this.query(indexSQL, (err, indexInfo) => cb(err, tableInfo, indexInfo));
    });
  }
  ...
  static paginateSQL(sql, orderBy, options) {
    ...
    // TODO: What the hell? If !orderBy, then use orderBy? Like this in official mysql connector too
    // if (!orderBy) {
  ...
  discoverDatabasesSchemas(options, cb) {
    this.execute(IBMiConnector.buildQuerySchemas(options), cb);
  }
...
exports.initialize = (ds, cb) => {
  ...
  return dataSource.connector.pool.init(cb);
};
