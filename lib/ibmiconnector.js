...
class IBMiConnector extends SQLConnector {
  ...
  buildColumnType(property) {
    ...
    switch (type) {
      ...
      case 'String':
        dt = 'VARCHAR';
        //dt = self.convertTextType(p, 'VARCHAR'); // TODO: loopback-connector-ibmi (old) has helper functions to add size
        break;
      case 'Number':
        dt = 'INTEGER';
        //dt = self.convertNumberType(p, 'INTEGER'); // TODO: loopback-connector-ibmi (old) has helper functions to add size
        break;
      case 'Date':
        dt = 'TIMESTAMP';
        break;
