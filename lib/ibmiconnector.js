...
class IBMiConnector extends SQLConnector {
  constructor(settings) {
    ...
    this.connectionString = IBMiConnector.buildConectionString(settings);
  ...
  static buildConectionString(settings)
  {
    let connectionString = '';
    if (settings.connectionString)
      connectionString = settings.connectionString;
    if (settings.dsn)
      connectionString = `DSN=${settings.dsn};${connectionString}`;
    if (settings.user) 
      connectionString = `${connectionString};UID=${settings.user}`;
    if (settings.password) {
      connectionString = `${connectionString};PWD=${settings.password}`;
    }
  ...
  static buildLimit(limit, offset) {
    let lim = limit;
    let off = offset;
  ...
  applyPagination(model, stmt, filter) {
    ...
    const limitClause = IBMiConnector.buildLimit(filter.limit, filter.offset ||
      filter.skip);
  ...
  static paginateSQL(sql, orderBy, options) {
    ...
    const limitAndOffset = IBMiConnector.buildLimit(limit, offset);

    return `${sql} ${orderBy ? `ORDER BY ${orderBy}` : ''}
      ${limitAndOffset}`;
  }
  ...
  */
  buildQueryTables(options) {
    ...
    return IBMiConnector.paginateSQL(sql, 'TABLE_SCHEMA, TABLE_NAME', options);
  }
  ...
  buildQueryColumns(schema, table) {
    return `SELECT
              ...
              CASE WHEN HAS_DEFAULT = 'I' THEN 1 ELSE 0 END AS "generated"
            FROM
              SYSIBM.SQLCOLUMNS
            WHERE
              TABLE_SCHEM = '${schema}'
