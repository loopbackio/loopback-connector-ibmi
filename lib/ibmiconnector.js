...
class IBMiConnector extends SQLConnector {
  ...
  toColumnValue(propertyDef, value) {
    debug('IBMiConnector.prototype.toColumnValue prop=%j value=%j',
      propertyDef, value);
    if (value === null) {
  ...
  buildInsert(model, data, options) {
    // if there is no data passed, we still need to default on the id
    if (Object.keys(data).length === 0) {
      data.id = null;
    }
    const fields = this.buildFields(model, data, false);
    ...
    insertStmt.sql = `SELECT * FROM FINAL TABLE(${insertStmt.sql})`;
  ...
  getInsertedId(model, info) {
    ...
    return info && typeof info[0][idColumn] === 'number' ?
      info[0][idColumn] : undefined;
  }
